...
@section('content')
<div class="container" id="admin-inquiries-archive" data-animate="animate__fadeInUp animate__delay-05s">
  <div class="admin-shell">
    ...
    <main class="admin-content">
      <div class="admin-topbar">
        <div class="admin-title">Inquiries Archive</div>
        <div class="admin-actions">
          <a href="/admin/inquiries" class="btn btn-login-secondary">← Back to Inquiries</a>
              ...
              <tbody>
                @forelse($items as $q)
                  <tr>
                    <td>{{ $q->name }}</td>
                    <td>{{ $q->email }}</td>
                    <td>{{ Str::limit($q->message, 100) }}</td>
                    <td>
                      <form method="POST" action="/admin/inquiries/{{ $q->id }}/unarchive" style="display:inline-block;">
                        @csrf
                ...
                @empty
                  <tr><td colspan="4">No archived inquiries.</td></tr>
                @endforelse
              </tbody>
            </table>
          </div>
